sap.ui.define([
	...
	return Controller.extend("web.webapp.controller.DetalleCuestionarios", {
		...
		someFunctionOfTheFirstController: function (sChannelId, sEventId, sData) {
		    ...
		    var that=this;
		    this.cuestionario_seleccionado=sData;
		    
		    var oModel = new sap.ui.model.odata.v2.ODataModel("/xsodata/riesgo.xsodata", false);
			oModel.read("/CuestionarioConcreto(" + sData.toString() + ")", {
		        success: function(results,oData,oResponse) {
					...
					that.getView().setModel(oODataJSONModelDLSet, "jsonmodel7");
					...
					if(results["FECHA_FIN"] !== null){
						...
						that.getView().byId("botonCambioTiempo").setVisible(false);
					}else{
						...
						that.getView().byId("botonCambioTiempo").setVisible(true);
					}
		        },
		        error: function() {
			...
			var filterByName = new sap.ui.model.Filter("ID_CUESTIONARIO", sap.ui.model.FilterOperator.Contains, sData);  
			filters.push(filterByName); 
			
		    oModel.read("/CuestionarioConcretoCalculos", {
		    	filters:filters,
		        success: function(results,oData,oResponse) {
					var oODataJSONModelDLSet = new sap.ui.model.json.JSONModel();
					oODataJSONModelDLSet.setData({ "DLSet": oData });
					that.getView().setModel(oODataJSONModelDLSet, "jsonmodel8");
		        },
		        error: function() {
		        	MessageToast.show("Recogida de informacion incorrecta");
		        }
